 ...
 * a^2 + b^2 = (c-1)(c+1)
 * The interesting part of this code is the two_factors function that takes as
 * input the prime decomposition of an integer n and outputs a set of all
 * pairs (a, b) such that a^2+b^2 = n.
 *)
let rec pow x n =
  if n = 0 then 1
...
let max_n = 75_000_000
let pd = prime_decomposition (max_n / 2 + 2)
...
let solutions_for_c c =
  let factors1, factors2 = pd (c-1), pd (c+1) in
  let pairs = two_squares (assoc_add (factors1, factors2)) in
  PS.fold (fun (a, b) acc -> acc + if a + b + c <= max_n then 1 else 0) pairs 0

let sum_seq f a b =
  let rec aux acc a = if a > b then acc else aux (f a + acc) (a+1) in
  aux 0 a

let () = Format.printf "Total: %d\n" (sum_seq solutions_for_c 2 (max_n/2))
